...
class TakePersonsWithStepAlgorithm implements PersonSelectionAlgorithm {
    ...
    public Person[] select(Person[] persons) {
        if(step == 1)
            return persons;
        int newLength = persons.length % step == 0 ? persons.length/step : persons.length/step + 1;
        Person[] newPersons = new Person[newLength];
        for( int i = 0, count = 0; i < persons.length; i++ ) {
            if(i%step == 0)
                newPersons[count++] = persons[i];
        }
        return newPersons;
    }
}
